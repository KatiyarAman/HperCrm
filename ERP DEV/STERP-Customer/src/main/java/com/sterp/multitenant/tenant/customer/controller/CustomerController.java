package com.sterp.multitenant.tenant.customer.controller;
...
public class CustomerController {
	...
	public ResponseEntity<Map<String, Object>> addOrUpdate(HttpServletRequest request,
			@RequestPart Customer object, @RequestPart(required = false) MultipartFile[] files) {
		return new ResponseEntity<Map<String, Object>>(this.customerService.addOrUpdateLeadSource(request ,object, files), HttpStatus.OK);

	}

	@GetMapping(value = "/getById/{customerId}/{moduleId}", produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<?> getById(@PathVariable("customerId") Long customerId,@PathVariable(value = "moduleId", required = true) Long moduleId) {
		return ResponseEntity.ok(this.customerService.getCustomerById(customerId, false));
	}

	
	@GetMapping(value = "/getSelectionList", produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<?> getSelectionList() {
		return ResponseEntity.ok(this.customerService.getSelectionList(false));
	}
	...
	public ResponseEntity<?> getList(HttpServletRequest httpServletRequest, @RequestParam Map<String, Object> params){
		...
		response.put("data", this.commonService.getCommonFilteredList(Customer.class, params));
		return ResponseEntity.ok(response);
	}
	
	@GetMapping(value = "/getByName/{name}", produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<?> getByName(@PathVariable("name") String name) {
		return ResponseEntity.ok(this.customerService.getCustomerByName(name, false));
	}
}
